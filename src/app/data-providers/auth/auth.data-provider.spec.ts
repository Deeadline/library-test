...
describe('AuthService', () => {
	...
	test('After login, authentication service property should contain user', () => {
		...
		dataProvider.user$.subscribe((_user: UserInterface) => expect(_user).toBe(user));
	});
	...
	test('After logout, authentication service property should not contain user', () => {
		dataProvider.logout();
		dataProvider.user$.subscribe((_user: UserInterface) => expect(_user).toBeNull());
	});

	test('getRole() should return value on logged user', () => {
		spyOn(dataProvider, 'user$').and.returnValue(user);
		dataProvider.getRole().subscribe((role: string) => expect(role).toEqual(user.role));
	});

	test('getRole() should be empty on unauthenticated user', () => {
		spyOn(dataProvider, 'user$').and.returnValue(null);
		dataProvider.getRole().subscribe((role: string) => expect(role).toBeNull());
	});

	test('getUsername() should return value on logged user', () => {
		spyOn(dataProvider, 'user$').and.returnValue(user);
		dataProvider.currentUser().subscribe((username: UserInterface) => expect(username).toEqual(user.username));
	});

	test('getUsername() should be empty on unauthenticated user', () => {
		spyOn(dataProvider, 'user$').and.returnValue(null);
		dataProvider.currentUser().subscribe((username: UserInterface) => expect(username).toBeNull());
	});
});
